...
for folder in range(1,(int(options['sporcNumber'])+1)):
    for count in range(1,(int(options['imageNumber']))):
        ...
        if str_to_bool(options['onlyWebcam']) == True:
            ...
            #leggo l'immagine
            img_grigio_matrice[folder,count] = core.img_in_matrice(core.scatta_foto(int(count), options, webcam), options)[0]
        else:
            #leggo l'immagine
            img_diff = Image.open(options['imagePath'] + str(folder) + '/' + str(count) + prefix + '.jpg', 'r')
            #converto in scala di grigi
            img_grigio_matrice[folder,count] = core.img_in_matrice(img_diff,options)[0]
        #core.conta_bin_e_range(img_grigio_matrice[folder,count])
...
if str_to_bool(options['onlyWebcam']) == True:
    ...
    i = 1
for count in range(i,(int(options['imageNumber']))):
